...
class Window:
    ...
    def MenuAbout(self):
        tkinter.messagebox.showinfo('Soto_Cleaner', '本软件版权归:朱海涛 所有,欢迎使用!')
  
    def MenuExit(self):
        self.root.quit()
    def MenuScanRubbish(self):
        result = tkinter.messagebox.askquestion('Soto_Cleaner', '扫描垃圾文件需较长时间,是否继续?')  
        if result == 'no':  
            return  
    ...
    def MenuDelRubbish(self):  
        result = tkinter.messagebox.askquestion('Soto_Cleaner', '删除垃圾文件时间较长,是否继续?')  
        if result == 'no':  
            return  
